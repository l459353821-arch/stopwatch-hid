...
| 模块 | 说明 |
|------|------|
| 🖥️ 圆形饼图触摸 | 三个扇形虚拟按键（复制 / 删除 / 粘贴），带高亮 + 马达震动反馈 |
...
- **震动马达**：M5IOE1 PWM1 → IO9（G9），固件用 M5.Power.setVibration(level) 驱动；供电走 3V3_L3B 电源轨（M5IOE1 G8/PYB_L3B_EN，固件启动时自动打开）
...
- 触摸命中时马达震动约 25ms；蓝色进入连删模式约 20ms；连删节拍每 100ms 震动 12ms；蓝色单击抬手 20ms
- 开机自检震动 ×2（各 150ms）：感到两次震动即马达正常
- 蓝色长按期间高亮持续显示，直到手指抬起
...
| 按键 | 动作 | 功能 |
|------|------|------|
| A（黄，GPIO2） | 按住 | 右 Option（⌥）按下；松开 | 释放右 Option |
| B（蓝，GPIO1） | 单击 | 回车（Return） |
| B（蓝） | 双击 | Shift+回车（换行，不发送消息） |
...
3. 配对成功后：
   ...
   - A 键按住 = 右 Option（按住说话，松开释放）；B 键单击 = 回车 / 双击 = Shift+回车（换行）/ 长按 = 撤销。
